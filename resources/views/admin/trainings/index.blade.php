                            ...
                            <form method="post" id="add_category_form" enctype="multipart/form-data">
                                ...
                                <div class="box-body">
                                    <div class="form-group">
                                        ...
                                        <textarea id="editor_training" name="short_description" rows="3" cols="160">{{$current->short_description}}</textarea>
                                    </div>
                                    <div class="form-group">
                                        <label>Long Description</label>
                                        <textarea id="editor_training1" name="long_description" rows="3" cols="160">{{$current->long_description}}</textarea>
                                    </div>
            ...
            <div class="row fileupload-buttonbar">
                <div class="col-lg-2">
                    ...
                    <span class="btn btn-success fileinput-button" style="display:block ; margin-bottom:10px">
                        ...
                        <input type="file" id="fileupload" name="photos[]" data-url="{{route('admin.trainings.upload')}}" multiple="">
                    </span>
                </div>
                <div class="col-lg-9">
            ...
            <div class="box box-warning box-solid collapsed-box">
                ...
                <div class="box-body">
                    ...
                    <table id="example" class="display" style="width:100%">
                        ...
                        <tbody>
                            @foreach ($training_images as $media)
                                <tr id="row{{$media->id}}">
                                    <td>{{$counter}}</td>
...
<script>
    ...
    $(document).ready(function(){
        CKEDITOR.replace('editor_training');
        CKEDITOR.replace('editor_training1');
        ...
        $('#add_category_form').on('submit', function(event){
            var short_description = CKEDITOR.instances.editor_training.getData();
            short_description = trimString(short_description);

            var long_description = CKEDITOR.instances.editor_training1.getData();
            long_description = trimString(long_description);

            event.preventDefault();
            $.ajax({
                url:"{{route('admin.trainings.update', [$current->id])}}",
                method:"POST",
        ...
        // $(".delete").click(function(){ // not working at page 2 see reason at https://www.gyrocode.com/articles/jquery-datatables-why-click-event-handler-does-not-work/
            if(confirm('Are you sure you want to Delete ?'))
            {
                ...
                $.ajax({
                    ...
                    url: '{{route("admin.trainings.delete")}}',
                    type: 'POST',
